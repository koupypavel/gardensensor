...
static const char *TAG = "XKSoilSensor";
const char *config_path = "/data/config.ini";
SemaphoreHandle_t xMutex = NULL;
bool discharged = false;

enum State esp_mode = DEEP_SLEEP;
uint8_t flags = 0;
int tdk_error = 0;
...
static void check_ota_partition()
{
    // lets init some stuff, blink for poweron and beep as well
    // Check OTA image, if valid preceed
...
static void printTime(struct timeval now)
{
    time_t t;
    struct tm *info;
    char buffer[64];
...
static void mqtt5_app_start(void)
{
    ...
    esp_mqtt_client_config_t mqtt5_cfg = {
        ...
        .credentials.authentication.password = "EcaijaeGh1soh3pah7iecaev8ooneidaet8haeheiS3ahv6giche4Vedee5Ahv1a",
        .session.last_will.topic = "/topic/will",
        .session.last_will.msg = "XKGardenSensor leaving",
...
void app_main(void)
{
    ...
    settings_st setup_struct;
    xMutex = xSemaphoreCreateMutex();

    //Decide what modde we should run
    ...
    case RUN:
    {
        ...
        mqtt5_app_start();
        ///////////////////////////////RUN xTasks
        xTaskCreatePinnedToCore(StatusTask, "StatusTask", 1024 * 4, NULL, tskIDLE_PRIORITY, NULL, 0);
    ...
    case CHARGING:
    {
        // once running deal with ota stuff
        check_ota_partition();
    ...
    {
        // if (web_run)
        // initWifiSoftap(setup_struct.wifi_ssid, setup_struct.wifi_key);
        xTaskCreatePinnedToCore(FileAccessTask, "FileAccessTask", 1024 * 16, NULL, configMAX_PRIORITIES - 4, NULL, 1);
        xTaskCreatePinnedToCore(dns_server_task, "DnsServer", 1024 * 4, NULL, configMAX_PRIORITIES - 4, NULL, 1);
        // xTaskCreatePinnedToCore(StatusTask, "StatusTask", 1024 * 4, NULL, tskIDLE_PRIORITY, NULL, 0);
        check_ota_partition();
