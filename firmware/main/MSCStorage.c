...
esp_err_t storage_mount(const char *base_path)
{
    ...
    FATFS *fs;
    err = esp_vfs_fat_register(base_path, drv, 3, &fs);
...
esp_err_t storage_unmount(void)
{
    ...
    s_base_path = NULL;
    s_fat_mounted = false;
    ESP_LOGW(TAG, "Storage unmounted succesfully");
...
esp_err_t storage_write_sector(size_t addr, size_t size, const void *src)
{
    ...
    if (s_fat_mounted) {
        ESP_LOGE(TAG, "can't write, FAT mounted1");
        return ESP_ERR_INVALID_STATE;
    }
    size_t sector_size = wl_sector_size(s_wl_handle);
    if (addr % sector_size != 0 || size % sector_size != 0) {
        //ESP_LOGE(TAG, "ESP_ERR_INVALID_ARG2, size:, sector_size:%d, addr: %d" sector_size, addr);
        return ESP_ERR_INVALID_ARG;
    }
    esp_err_t err = wl_erase_range(s_wl_handle, addr, size);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "wl_erase_range failed (0x%x)3", err);
        return err;
    }
